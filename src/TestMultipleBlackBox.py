...
class TestMultipleBlackBox:
    def read_excel_yield(self, filename, starting_row, end_row) -> Iterator[pd.DataFrame]:
        yield pd.read_excel(filename, skiprows=starting_row , nrows=end_row+1)

    def __init__(self, file_name, first_row, row_count) -> None:
    ...
    def getDataFromExcel(self, line_index):
    ...
    def getResults(self, doPlots = True):
        print()
